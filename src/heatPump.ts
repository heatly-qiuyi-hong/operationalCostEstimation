...
async function heatPumpConsumption(
  gasConsumption: number[],
  weatherDataHourly: number[],
  ...
  const flowTemp = weatherDataHourly.map(
    (temp: number) => slope * temp + intercept,
  );
  ...
  for (let i = 0; i < weatherDataHourly.length; i++) {
    console.log(
      `Outdoor Temp: ${weatherDataHourly[i]}°C, Flow Temp: ${flowTemp[i]}°C`,
    );
    ...
    async function getUser() {
      try {
        ...
        const response = await axios.get(
          `https://dev01.heatly.tech/v1/heatpump/metrics/${params.copOutputKey}`,
          {
            params: params,
            headers: headers,
          },
        ...
        const copValue = response.data.find(
          (item: any) => item.manufacturerId === manufacturerId,
        )?.metrics?.cop;
        return copValue;
      } catch (error) {
        console.error(error);
...
async function HPConsumption(
  ...
  try {
    const response = await axios.get(
      `https://dev01.heatly.tech/v1/heatpump/${manufacturerId}`,
      {
    ...
    for (let i = 0; i < weatherDataHourly.length; i++) {
      const copValue = copData.find(
        (item: any) =>
          item.key ===
          `A${Math.round(weatherDataHourly[i])}W${Math.round(flowTemp[i])}`,
      )?.value?.cop;
